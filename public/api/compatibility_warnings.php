...
function makeWarning(array $organism, string $message): array
{
    return [
...
try {
    ...
    foreach ($organisms as $o) {
        ...
        if ($o['min_tank_size_gallons'] !== null && $o['volume_gallons'] !== null
            ...
            $warnings[] = makeWarning($o, "This tank may be too small — {$o['species_common_name']} needs at least {$o['min_tank_size_gallons']} gallons.");
        }

        if ($o['temperament'] && in_array($o['temperament'], ['Aggressive', 'Semi-aggressive'], true) && count($tankMates) > 1) {
            $warnings[] = makeWarning($o, "{$o['species_common_name']} can be {$o['temperament']} — keep an eye on tankmates.");
        }

        if ($o['min_group_size'] !== null && (int) $o['min_group_size'] > 1 && $sameSpeciesCount < (int) $o['min_group_size']) {
            $warnings[] = makeWarning($o, "{$o['species_common_name']} does best in groups of {$o['min_group_size']} or more (currently {$sameSpeciesCount}).");
        }

        if ($o['is_shrimp_safe'] !== null && !$o['is_shrimp_safe'] && $hasInvertebrate) {
            $warnings[] = makeWarning($o, "{$o['species_common_name']} may not be safe with the invertebrates in this tank.");
        }

        if ($o['is_plant_safe'] !== null && !$o['is_plant_safe'] && $hasPlant) {
            $warnings[] = makeWarning($o, "{$o['species_common_name']} may eat the plants in this tank.");
        }
        ...
        if ($temp !== null && $o['min_temp_f'] !== null && $o['max_temp_f'] !== null
            ...
            $warnings[] = makeWarning($o, "Tank temperature ({$temp}°F) is outside {$o['species_common_name']}'s comfortable range ({$o['min_temp_f']}-{$o['max_temp_f']}°F).");
        }

        $ph = $latestByTankParam[$o['tank_id'] . '|pH'] ?? null;
        if ($ph !== null && $o['min_ph'] !== null && $o['max_ph'] !== null
            && ($ph < (float) $o['min_ph'] || $ph > (float) $o['max_ph'])) {
            $warnings[] = makeWarning($o, "Tank pH ({$ph}) is outside {$o['species_common_name']}'s comfortable range ({$o['min_ph']}-{$o['max_ph']}).");
        }

        if ($o['care_level'] && $o['care_level'] !== 'Beginner') {
            $warnings[] = makeWarning($o, "{$o['species_common_name']} is rated {$o['care_level']} care — good to research before committing.");
        }
    }

    foreach ($organismsByTank as $tankId => $mates) {
        $totalInches = 0.0;
        $hasSize = false;
